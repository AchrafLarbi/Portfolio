...
const ShowCard = ({ activeId, setShowCard }) => {
  ...
  return (
    <div className="w-full h-screen fixed top-0 left-0 z-10 bg-[#0a192f] backdrop-filter backdrop-blur-sm bg-opacity-10 font-sans">
      <div
        className="max-w-[1000px] w-[80%] sm:w-[60%] lg:w-[40%] mx-auto absolute top-1/2 left-1/2 z-20 bg-white rounded-[8px]
      ...
      >
        ...
        <div>
          <h2 className="text-xl sm:text-2xl text-[#0a192f] font-[700] my-5">
            {portfolio.title}
          </h2>
          <p className="text-[12px] sm:text-[15px] text-[#4d4d4d]">
            {portfolio.description}
          </p>
        </div>
        <div className="mt-5 flex items-center gap-3 flex-wrap">
          <h4 className="text-[14px] sm:text-[18px] text-[#0a192f] text-[700]">
            Technologies:
          </h4>
        ...
        {/* Conditional rendering for code source links */}
        {portfolio.BackendCodeSource && (
          <a
            target="_blank"
            rel="noreferrer"
            href={portfolio.BackendCodeSource}
            className="mr-4"
          >
        ...
        {portfolio.FrontendCodeSource && (
          <a
            target="_blank"
            rel="noreferrer"
            href={portfolio.FrontendCodeSource}
          >
            <button className="text-white bg-[#0a192f] hover:bg-[#53565a] py-2 px-4 my-4 rounded-[8px] text-[12px] sm:text-[14px] ease-in duration-200">
        ...
        {/* Check for CodeSource and render it if exists */}
        {portfolio.CodeSource && (
          <a target="_blank" rel="noreferrer" href={portfolio.CodeSource}>
            <button className="text-white bg-[#0a192f] hover:bg-[#53565a] py-2 px-4 my-4 rounded-[8px] text-[12px] sm:text-[14px] ease-in duration-200">
              Code Source
            </button>
          </a>
